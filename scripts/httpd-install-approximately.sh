...
: "${1:?Define Apache2 version or URL and try again}"

install_httpd() {
  local full_version httpd_revision
  local desired="${1}"
  ...
  if __validate_url "${desired}"; then
    ...
    ## Download and install httpd from URL
    curl --silent \
      "${desired}" \
      | tar -C "/" --strip-components 1 -zx

    ## Remove deps
    apt-env.sh apt-remove.sh curl

    ## Define variables for /etc/environment
    httpd_revision='Installed-from-URL'
  ...
  ## Else installed base component form aptitude source
    ...
    ## Search httpd version on repository if received version is approximately
    full_version=$(apt-cache show apache2 \
      | grep -E '^Version:' \
      | grep "${desired}" \
      | sort -rV \
      | head -n1 \
    ...
    [[ -n ${full_version} ]] || {
      echo "[Error]: Could not find Apache2 version matching '${desired}'" >&2
      return 1
    }
    ...
    apt-install.sh \
      ...
      "apache2=${full_version}"

    ## Define variables for /etc/environment
    httpd_revision="${full_version}"

    ## Disabling mandatory authentication (authorization) in the service configuration
    {
      echo 'AstraMode off'
      echo 'AstraAudit off'
    } >>/etc/apache2/apache2.conf

    ## Create soft link apache2 -> httpd
    ln -sf "$(which apache2)" /usr/local/bin/httpd

    ## Remove mess config from available/enabled conf
  ...
  {
    echo "HTTPD_REVISION=${httpd_revision}"
